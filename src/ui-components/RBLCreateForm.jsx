...
export default function RBLCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const rBL = await DataStore.save(new RBL(modelFieldsToSave));
          const promises = [];
          promises.push(
            ...Children.reduce((promises, original) => {
              promises.push(
                DataStore.save(
      ...
      {...getOverrideProps(overrides, "RBLCreateForm")}
      {...rest}
    >
      <TextField
        label="Last name"
